...
for irz = 1:nrz
    ...
    w_smooth = milca_weights(cov1ko[kν, kν, :], ones(length(kν)), B)
    cl1[:, irz] = ilc_clean_cij(cov1ko[kν, kν, :], w_smooth)
    cl2[:, irz] = ilc_clean_cij(cov2ko[kν, kν, :], w_smooth)
    ...
    if iib ≠ []
        for ib = 1:maximum(iib)
            ...
            w = milca_weights(cov1ko[kν, kν, ib], ones(length(kν)), B)
            cl1[ib, irz] = ilc_clean_cij(cov1ko[kν, kν, ib], w)
            cl2[ib, irz] = ilc_clean_cij(cov2ko[kν, kν, ib], w)
...
CSV.write("milca_results_removelowell.csv", t, header = ["irz", "r_wo_FGmarg", "r_w_FGmarg"])
