...
for i in range (0,10):
    ...
    # RING format
    hp.write_map("map_equ_{:03d}ghz_r8_uKcmb.fits".format(nu[i]), map_equ, coord="C")
    # NESTED format
    map_nested = hp.reorder(map_equ, r2n=True)
    hp.write_map("map_equ_{:03d}ghz_r8_nested_uKcmb.fits".format(nu[i]), map_nested, coord="C", nest=True)
