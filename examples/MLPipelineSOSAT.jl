...
using Tables, CSV
# %% Multi-threading computation for maximisation of the likelihood?
multithreads = true
if multithreads
    using Base.Threads
    println("# of threads available = ", nthreads())
end
...
for irz = 1:nrz
    ...
    d = zeros(nν, 12 * nside^2, 2) # [Q(ν, ip), U(ν, ip)] data matrix
    for iν = 1:nν
        ...
        d[iν, :, 1], d[iν, :, 2] = f_q[iν] + c_q + n_q, f_u[iν] + c_u + n_u
        # f2: noise
    ...
    ## Maximise the profile likelihood for the foreground parameters
    # Frequency response matrix and derivatives
    A(x) = [ones(nν) synch.(ν, βs = x[1]) dust1.(ν, βd = x[2])]
    ...
    we, wb = milca_weights(ce2, ones(nν), B), milca_weights(cb2, ones(nν), B)
