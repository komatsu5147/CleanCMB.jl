...
for irz = 1:nrz
    ...
    B = [synch.(ν, βs = β[1]) dust1.(ν, βd = β[2], Td = β[3])] # Best-fitting frequency response of the FG
    ...
    # SO only
    ASO(x) = [ones(nνSO) synch.(ν[1:nνSO], βs = x[1]) dust1.(
        ν[1:nνSO],
        βd = x[2],
        Td = x[3],
    ...
    func_sumSO(x, cij) =
        -lnlike_fgprior(x) - sum(
            (2 * ell_eff[jb] + 1) * loglike_beta(
                ...
                ASO(x),
                cij[1:nνSO, 1:nνSO, jb],
            ) for jb = 1:nbands
        ) # -log(likelihood) to minimise by `optimize`
    res = optimize(x -> func_sumSO(x, cbij), β0)
    β = Optim.minimizer(res)
    B = [synch.(ν[1:nνSO], βs = β[1]) dust1.(ν[1:nνSO], βd = β[2], Td = β[3])] # Best-fitting frequency response of the FG
    wb_smooth = milca_weights(cb1[1:nνSO, 1:nνSO, :], ones(nνSO), B)
    res = optimize(x -> func_sumSO(x, ceij), β0)
    β = Optim.minimizer(res)
    B = [synch.(ν[1:nνSO], βs = β[1]) dust1.(ν[1:nνSO], βd = β[2], Td = β[3])] # Best-fitting frequency response of the FG
    we_smooth = milca_weights(ce1[1:nνSO, 1:nνSO, :], ones(nνSO), B)
    ee1SO[:, irz], bb1SO[:, irz] =
        ilc_clean_cij(ce1[1:nνSO, 1:nνSO, :], we_smooth),
        ilc_clean_cij(cb1[1:nνSO, 1:nνSO, :], wb_smooth)
    ee2SO[:, irz], bb2SO[:, irz] =
        ilc_clean_cij(ce2[1:nνSO, 1:nνSO, :], we_smooth),
        ilc_clean_cij(cb2[1:nνSO, 1:nνSO, :], wb_smooth)
    ee3SO[:, irz], bb3SO[:, irz] =
        ilc_clean_cij(ce3[1:nνSO, 1:nνSO, :], we_smooth),
        ilc_clean_cij(cb3[1:nνSO, 1:nνSO, :], wb_smooth)
    ...
    if iib ≠ []
        for ib = 1:maximum(iib)
            ...
            we = milca_weights(ce1[1:nνSO, 1:nνSO, ib], ones(nνSO), B)
            ee1SO[ib, irz], bb1SO[ib, irz] =
                ilc_clean_cij(ce1[1:nνSO, 1:nνSO, ib], we),
                ilc_clean_cij(cb1[1:nνSO, 1:nνSO, ib], wb)
            ee2SO[ib, irz], bb2SO[ib, irz] =
                ilc_clean_cij(ce2[1:nνSO, 1:nνSO, ib], we),
                ilc_clean_cij(cb2[1:nνSO, 1:nνSO, ib], wb)
            ee3SO[ib, irz], bb3SO[ib, irz] =
                ilc_clean_cij(ce3[1:nνSO, 1:nνSO, ib], we),
                ilc_clean_cij(cb3[1:nνSO, 1:nνSO, ib], wb)
        end
    end
...
# Plot and save to "milca_clbb_sim_sosatccatp.pdf"
...
if showresults
    ...
    savefig("milca_clbb_sim_sosatccatp.pdf")
    display(p)
end
